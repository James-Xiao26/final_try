...
**Market Analytics page** (`web/app/market/[conditionId]/page.tsx`) is the per-market deep dive, reachable by clicking any market on the Markets/Convergence/Resolved surfaces. `getMarketAnalytics(conditionId)` (supabase.ts) assembles the `markets` snapshot, the daily YES price series (from `market_price_history` via the YES outcome token), the leaderboard `CrowdMarketDetail`, and the tracked fills joined with each wallet's identity; the page derives every chart/metric at read time via the pure `web/lib/marketAnalytics.ts` helpers (see test list above). Sections: an interactive hand-rolled SVG `PriceChart` (hover crosshair/tooltip + whale-trade overlay + regime-shift markers), `WhaleFeed`, `ConcentrationChart`, `PnlHistogram`, a 3-way smart-money lean (headcount/capital/skill), the reused `ConvergenceChart`, and `CrowdParticipants`. Both `meta` and `detail` are independently nullable so the page degrades gracefully. Markets become clickable only once `markets.condition_id` is populated — `mapEvent` now stores the leading market's conditionId, but a fresh `ingest:markets` run (+ Heroku deploy) is required to backfill it; until then those rows fall back to plain text + an external Polymarket link.
